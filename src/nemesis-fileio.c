/*
 * $Id: nemesis-fileio.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
static int nemesis_readfile(uint8_t *memory, const char *file, 
    const size_t maxsize, const uint32_t mode);
 ...
 **/
static int
nemesis_readfile(uint8_t *memory, const char *file, const size_t maxsize, 
    const uint32_t mode)
{
 ...
 **/
int
builddatafromfile(const size_t buffsize, FileData *memory, const char *file, 
    const uint32_t mode)
{

	if ((memory->file_mem = (uint8_t *)calloc(buffsize, 
	    sizeof(uint8_t))) == NULL) {
		if (mode == PAYLOADMODE)
			perror("ERROR: Unable to allocate packet payload "
			    "memory");
		else
			perror("ERROR: Unable to allocate packet options "
			    "memory");
        	return (-1);
	}

	if ((memory->file_s = nemesis_readfile(memory->file_mem, file, 
	    buffsize, (mode == PAYLOADMODE ? PAYLOADMODE : OPTIONSMODE))) < 0) {

		if (mode == PAYLOADMODE)
			fprintf(stderr, "ERROR: Unable to read any payload "
			    "data.\n");
		else
			fprintf(stderr, "ERROR: Unable to read any options "
			    "data.\n");
		return (-1);
	}
	return (0);
