/*
 * $Id: nemesis-igmp.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
void
nemesis_igmp(int argc, char **argv)
{
...
igmp_validatedata(void)
{
	...
	if (!got_group) {
		nemesis_name_resolve("224.0.0.1", 
		    (uint32_t *)&igmphdr.igmp_group.s_addr);
		nemesis_name_resolve("224.0.0.1", 
...
static void
igmp_cmdline(int argc, char **argv)
{
	int opt, i;
	uint32_t addr_tmp[6];
	char *igmp_options;
	...
	while ((opt = getopt(argc, argv, igmp_options)) != -1) {
		switch (opt) {
		case 'c':	/* IGMP code */
			if (getint8(optarg, &igmphdr.igmp_code) < 0)
				igmp_usage(argv[0]);
			got_code = 1;
			break;
		...
		case 'i':	/* IGMP group address */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&igmphdr.igmp_group.s_addr)) < 0) {
				fprintf(stderr, "ERROR: Invalid IGMP group "
				    "address: \"%s\".\n", optarg);
				igmp_usage(argv[0]);
			}
			got_group = 1;
		...
		case 'O':	/* IP options file */
			...
			break;
		case 'p':	/* IGMP type */
			if (getint8(optarg, &igmphdr.igmp_type) < 0)
				igmp_usage(argv[0]);
			got_type = 1;
			break;
		case 'P':	/* payload file */
