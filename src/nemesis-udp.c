/*
 * $Id: nemesis-udp.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
void
nemesis_udp(int argc, char **argv)
{
	...
	if (got_payload) {
		...
		if (builddatafromfile(((got_link == 1) ? UDP_LINKBUFFSIZE :
		    UDP_RAWBUFFSIZE), &pd, (const char *)payloadfile, 
		    (const uint32_t)PAYLOADMODE) < 0) {
#endif
			udp_exit(1);
		}
...
static void
udp_cmdline(int argc, char **argv)
{
	int opt, i;
	uint32_t addr_tmp[6];
	char *udp_options;
