/*
 * $Id: nemesis-proto_udp.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
int
buildudp(ETHERhdr *eth, IPhdr *ip, UDPhdr *udp, FileData *pd, FileData *ipod, 
    char *device)
{
	int n;
	...
	if (n != udp_packetlen) {
		...
		if (verbose) {
			if (got_link)
				printf("Wrote %d byte UDP packet through "
				    "linktype %s.\n", n, 
