/*
 * $Id: nemesis-proto_dns.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
int
builddns(ETHERhdr *eth, IPhdr *ip, TCPhdr *tcp, UDPhdr *udp, DNShdr *dns, 
    FileData *pd, FileData *ipod, FileData *tcpod, char *device) 
{
	...
	if (verbose == 2)
		nemesis_hexdump((char *)pkt, dns_packetlen, HEX_ASCII_DECODE);
	if (verbose >= 3)
		nemesis_hexdump((char *)pkt, dns_packetlen, HEX_RAW_DECODE);
