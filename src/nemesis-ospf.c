/*
 * $Id: nemesis-ospf.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
void
nemesis_ospf(int argc, char **argv)
{
...
ospf_validatedata(void)
{
	...
	if (got_mode > 1) {
		fprintf(stderr, "ERROR: OSPF injection mode multiply "
...
static void
ospf_cmdline(int argc, char **argv)
{
	int opt, i;
	uint32_t addr_tmp[6];
	char *ospf_options;
	...
	while ((opt = getopt(argc, argv, ospf_options)) != -1) {
		...
		case 'a':	/* OSPF authorization type */
			tmp16 = 0;
			if (getint16(optarg, &tmp16) < 0)
				ospf_usage(argv[0]);
			else
				ospfhdr.ospf_auth_type = (uint16_t)htons(tmp16);
			break;
		case 'A':	/* OSPF area ID */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&ospfhdr.ospf_area_id.s_addr)) < 0) {
				fprintf(stderr, "ERROR: Invalid OSPF area ID "
				    "IP address: \"%s\".\n", optarg);
				ospf_usage(argv[0]);
			}
			break;
		case 'B':	/*
				 * OSPF number of broadcasted link state 
				 * advertisements.
				 */
			if (getint32(optarg, &tmp32) < 0)
				ospf_usage(argv[0]);
			else
				lsuhdr.lsu_num = (uint32_t)htonl(tmp32);
			break;
		...
		case 'f':	/* external AS LSA forwarding IP address */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&aslsahdr.as_fwd_addr.s_addr)) < 0) {
				fprintf(stderr, "ERROR: Invalid external LSA "
				    "forwarding IP address: \"%s\".\n", optarg);
				ospf_usage(argv[0]);
			}
			break;
		...
		case 'g':	/* OSPF external route tag */
			if (getint32(optarg, &tmp32) < 0)
				...
				aslsahdr.as_rte_tag = (uint32_t)htonl(tmp32);
			break;
		case 'G':	/* 
				 * OSPF link state acknowledgment age in 
				 * seconds.
				 */
			if (getint16(optarg, &tmp16) < 0)
				ospf_usage(argv[0]);
			else
				lsahdr.lsa_age = (uint16_t)htons(tmp16);
			break;
		case 'H':	/* Ethernet source address */
			...
			break;
		case 'i':	/* 
				 * OSPF HELLO link state countdown timer in 
				 * seconds.
				 */
			if (getint32(optarg, &tmp32) < 0)
				ospf_usage(argv[0]);
			else 
				ospfhellohdr.hello_dead_intvl = (uint32_t)htonl(tmp32);
			break;
		case 'I':	/* IP ID */
		...
		case 'k':	/* OSPF link state acknowledgment link data */
			if (getint32(optarg, &tmp32) < 0)
				...
				rtrlsahdr.rtr_link_data = (uint32_t)htonl(tmp32);
			break;
		case 'l':	/* 
				 * OSPF HELLO las packet interval in seconds.
				 */ 
			if (getint16(optarg, &tmp16) < 0)
				ospf_usage(argv[0]);
			else
				ospfhellohdr.hello_intrvl = (uint16_t)htons(tmp16);
			break;
		case 'L':	/* OSPF link state request ID */
			if (getint32(optarg, &tmp32) < 0)
				ospf_usage(argv[0]);
			else
				lsrhdr.lsr_lsid = (uint32_t)htonl(tmp32);
			break;
		case 'm':	/* OSPF link state acknowledgment link metric */
			if (getint16(optarg, &tmp16) < 0)
				ospf_usage(argv[0]);
			else
				rtrlsahdr.rtr_metric = (uint16_t)htons(tmp16);
			break;
		...
		case 'n':	/* OSPF multi-purpose netmask placement */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&ospfhellohdr.hello_nmask.s_addr)) < 0) {
				fprintf(stderr, "ERROR: Invalid netmask IP "
				    "address: \"%s\".\n", optarg);
				ospf_usage(argv[0]);
			} else {
		...
		case 'N':	/* OSPF HELLO neighbor router */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&ospfhellohdr.hello_nbr.s_addr)) < 0) {
				fprintf(stderr, "ERROR: Invalid OSPF HELLO "
				    "neighbor IP address: \"%s\".\n", optarg);
				ospf_usage(argv[0]);
			}
			break;
		case 'o':	/* OSPF multi purpose options */
			if (getint8(optarg, &ospfhellohdr.hello_opts) < 0)
				ospf_usage(argv[0]);
			else
				dbdhdr.dbd_opts = lsahdr.lsa_opts = ospfhellohdr.hello_opts;
			break;
		...
		case 'p':	/* OSPF injection mode */
			if (strlen(optarg) == 1) {
				...
				fprintf(stderr, "ERROR: Invalide OSPF "
				    "injection mode: %s.\n", optarg);
				ospf_usage(argv[0]);
			}
			switch (cmd_mode) {
		...
		case 'r':	/* OSPF advertising router ID */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&lsahdr.lsa_adv.s_addr)) < 0) {
				fprintf(stderr, "ERROR: Invalid OSPF "
				    "advertising router ID (IP address): "
				    "\"%s\".\n", optarg);
				ospf_usage(argv[0]);
			}
			break;
		case 'R':	/* OSPF source router ID */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&ospfhdr.ospf_rtr_id.s_addr)) < 0) {
				fprintf(stderr, "ERROR: Invalid OSPF "
				    "source router ID (IP address): \"%s\".\n", 
				    optarg);
				ospf_usage(argv[0]);
			}
			break;
		case 's':	/* OSPF DBD sequence number */
			if (getint32(optarg, &tmp32) < 0)
				...
				dbdhdr.dbd_seq = (uint32_t)htonl(tmp32);
			break;
		...
		case 'u':	/* OSPF number of links in link state header */
			if (getint16(optarg, &tmp16) < 0)
				...
				rtrlsahdr.rtr_num = (uint16_t)htons(tmp16);
			break;
		case 'v':
			if (++verbose >= 1)
				nemesis_printtitle((const char *)title);
		...
		case 'x':	/* OSPF DBD exchange type */
			if (getint8(optarg, &dbdhdr.dbd_type) < 0)
				ospf_usage(argv[0]);
			break;
		case 'y':	/* OSPF description of router link */
			if (getint8(optarg, &rtrlsahdr.rtr_type) < 0)
				ospf_usage(argv[0]);
			break;
		case 'z':	/* OSPF DBD interface MTU size */
			if (getint16(optarg, &tmp16) < 0)
				...
				dbdhdr.dbd_mtu_len = (uint16_t)htons(tmp16);
			break;
#if defined(WIN32)
		case 'Z':
