/*
 * $Id: nemesis-arp.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
void
nemesis_arp(int argc, char **argv)
{
...
arp_initdata(void)
{
	...
	memset(arphdr.ar_spa, 0, 4);		/* ARP sender proto (IP) addr */
	memset(arphdr.ar_tha, 0, 6);		/* ARP frame target address */
	memset(arphdr.ar_tpa, 0, 4);		/* ARP target proto. (IP) addr */
	pd.file_mem = NULL;
	pd.file_s = 0;
	return;
}
...
static void
arp_validatedata(void)
{
	struct sockaddr_in sin;

	/* validation tests */
	if ((!memcmp(arphdr.ar_spa, zero, 4)) || 
	    (!memcmp(arphdr.ar_tpa, zero, 4))) {
	...
	if (solarismode && arp_dst) {
		fprintf(stderr, "ERROR: Using -s and -m is redundant, choose "
		    "one or the other.\n");
		arp_exit(1);
	}
	...
	/* 
	 * For RARP functionality, set the appropriate opcode in the ARP 
	 * frame.
	 */
	if (rarp) {
		if (reply)
...
arp_usage(char *arg)
{
	...
	printf("ARP/RARP Options: \n"
	       ...
	       "  -s <Solaris style ARP requests with target hardware addess "
	       "set to broadcast>\n"
...
static void
arp_cmdline(int argc, char **argv)
{
	int opt, i;
	uint32_t addr_tmp[6];
	char *arp_options;
	...
	while ((opt = getopt(argc, argv, arp_options)) != -1) {
		...
		case 'h':	/* ARP sender hardware address */
			...
			for (i = 0; i < 6; i++) 
				arphdr.ar_sha[i] = (uint8_t)addr_tmp[i];
			break;
		...
		case 'm':	/* ARP target hardware address */
			...
			sscanf(optarg, "%02X:%02X:%02X:%02X:%02X:%02X", 
			    ...
			    &addr_tmp[3], &addr_tmp[4], &addr_tmp[5]);
			for (i = 0; i < 6; i++)
				arphdr.ar_tha[i] = (uint8_t)addr_tmp[i];
			break; 
