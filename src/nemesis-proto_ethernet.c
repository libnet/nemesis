/*
 * $Id: nemesis-proto_ethernet.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
int
buildether(ETHERhdr *eth, FileData *pd, char *device)
{
	int n;
	...
	libnet_build_ethernet(eth->ether_dhost, eth->ether_shost, 
	    eth->ether_type, pd->file_mem, pd->file_s, pkt);
...
#endif
	if (verbose == 2)
		nemesis_hexdump((char *)pkt, eth_packetlen, HEX_ASCII_DECODE);
	if (verbose >= 3)
		nemesis_hexdump((char *)pkt, eth_packetlen, HEX_RAW_DECODE);
	...
	if (verbose) {
		if (ethertype != NULL)
			printf("Wrote %d byte Ethernet type %s packet through "
			    "linktype %s.\n", n, ethertype, 
			    nemesis_lookup_linktype(l2->linktype));
		else
			printf("Wrote %d byte Ethernet type %hu packet through "
			    "linktype %s.\n", n, eth->ether_type, 
			    nemesis_lookup_linktype(l2->linktype));
	}
