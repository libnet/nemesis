...
{		      
    (void)libnet_build_ospfv2_hello(
          ospfhellohdr.hello_nmask.s_addr,                         /* netmask */
...
//          ospfhellohdr.hello_nbr.s_addr,                          /* neighbor */
    ...
    /* authentication data */
    (void)libnet_build_data(
          auth,                                       /* auth data */
          LIBNET_OSPF_AUTH_H,                         /* payload size */
          l,                                          /* libnet handle */
          0);                                         /* libnet id */

    (void)libnet_build_ospfv2(
          LIBNET_OSPF_HELLO_H + LIBNET_OSPF_AUTH_H + pd->file_s,   /* OSPF packet length */
...
{
    (void)libnet_build_ospfv2_lsa_rtr(
          rtrlsahdr.rtr_flags, 
          rtrlsahdr.rtr_num, 
...
{
    (void)libnet_build_ospfv2_lsa_net(
          netlsahdr.net_nmask.s_addr, 
...
{
    (void)libnet_build_ospfv2_lsa_sum(
          sumlsahdr.sum_nmask.s_addr, 
...
{
    (void)libnet_build_ospfv2_lsa_as(
          aslsahdr.as_nmask.s_addr, 
          aslsahdr.as_metric, 
...
int buildospf(ETHERhdr *eth, IPhdr *ip, FileData *pd, FileData *ipod, libnet_t *l, int got_type)
{
    ...
    (void)libnet_build_ipv4(ospf_meta_packetlen, 
                            ip->ip_tos, 
                            ip->ip_id, 
                            ip->ip_off, 
                            ip->ip_ttl, 
