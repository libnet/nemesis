/*
 * $Id: nemesis-proto_rip.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT 
...
int
buildrip(ETHERhdr *eth, IPhdr *ip, UDPhdr *udp, RIPhdr *rip, FileData *pd, 
    FileData *ipod, char *device)
{
	...
	rip_packetlen = link_offset + LIBNET_IP_H + LIBNET_UDP_H + 
	    LIBNET_RIP_H + pd->file_s + ipod->file_s;
	...
	libnet_build_udp(udp->uh_sport, udp->uh_dport, NULL, 0, pkt + 
	...
	if (n != rip_packetlen) {
		...
		if (verbose) {
			if (got_link)
				printf("Wrote %d byte RIP packet through "
				    "linktype %s.\n", n, 
