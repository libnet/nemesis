/*
 * $Id: nemesis-ethernet.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
void
nemesis_ethernet(int argc, char **argv)
{
	...
	if (got_payload) {
		if (builddatafromfile(ETHERBUFFSIZE, &pd, 
...
static void
ethernet_cmdline(int argc, char **argv)
{
	int opt, i;
	uint32_t addr_tmp[6];
	...
	while ((opt = getopt(argc, argv, ethernet_options)) != -1) {
		...
		case 'P':	/* payload file */
			...
			break;
		case 'T':	/* Ethernet type */
			if ((getint16(optarg, &etherhdr.ether_type)) < 0)
				ethernet_usage(argv[0]);
			break;
		case 'v':
			if (++verbose >= 1)
