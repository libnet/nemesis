/*
 * $Id: nemesis-proto_arp.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
int
buildarp(ETHERhdr *eth, ARPhdr *arp, FileData *pd, char *device, int reply)
{
	int n = 0;
	...
	if (verbose == 2)
		nemesis_hexdump((char *)pkt, arp_packetlen, HEX_ASCII_DECODE);
	if (verbose >= 3)
		nemesis_hexdump((char *)pkt, arp_packetlen, HEX_RAW_DECODE);
	...
	if (n != arp_packetlen) {
		...
		if (verbose) {
			if (memcmp(eth->ether_dhost, (void *)&one, 6)) {
				printf("Wrote %d byte unicast ARP request "
				    "packet through linktype %s.\n", n, 
				    nemesis_lookup_linktype(l2->linktype));
			} else { 
				printf("Wrote %d byte %s packet through "
				    "linktype %s.\n", n, 
