/*
 * $Id: nemesis-ip.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
nemesis_ip(int argc, char **argv)
{
	...
	if (got_payload) {
#if defined(WIN32)
		if (builddatafromfile(IP_LINKBUFFSIZE, &pd, 
...
ip_validatedata(void)
{
	...
	if (memcmp(etherhdr.ether_shost, zero, 6) && !got_link && !device) {
		if (libnet_select_device(&sin, &device, (char *)&errbuf) < 0) {
			printf("ERROR: Device not specified and unable to "
...
ip_cmdline(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, ip_options)) != -1) {
		...
		case 'O':	/* IP options file */
			...
			break;
		case 'p':	/* IP protocol */
			if (getint8(optarg, &iphdr.ip_p) < 0)
				ip_usage(argv[0]);
			break;
		case 'P':	/* payload file */
			if (strlen(optarg) < 256) {
