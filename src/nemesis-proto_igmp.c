/*
 * $Id: nemesis-proto_igmp.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
int
buildigmp(ETHERhdr *eth, IPhdr *ip, IGMPhdr *igmp, FileData *pd,
    FileData *ipod, char *device)
{
	...
	if (n != igmp_packetlen) {
		...
		if (verbose) {
			if (got_link)
				printf("Wrote %d byte IGMP packet through "
				    "linktype %s.\n", n, 
