...
int
buildicmp(ETHERhdr *eth, IPhdr *ip, ICMPhdr *icmp, IPhdr *ipunreach, FileData *pd, FileData *ipod, FileData *origod, libnet_t *l)
{
    int n;
    ...
    case ICMP_MASKREQ:
        libnet_build_icmpv4_mask(icmp->icmp_type, icmp->icmp_code, 0,
                                 icmp->hun.echo.id, icmp->hun.echo.seq,
                                 icmp->dun.mask, pd->file_mem, pd->file_s, l, 0);
        break;
    case ICMP_TSTAMP:
    ...
    case ICMP_UNREACH:
    case ICMP_TIMXCEED:
        libnet_build_icmpv4_unreach(icmp->icmp_type, icmp->icmp_code, 0,
                                    pd->file_mem, pd->file_s, l, 0);
        break;
    case ICMP_REDIRECT:
        libnet_build_icmpv4_redirect(icmp->icmp_type, icmp->icmp_code, 0,
                                     ntohl(icmp->hun.gateway),
                                     pd->file_mem, pd->file_s, l, 0);
        break;
    }
