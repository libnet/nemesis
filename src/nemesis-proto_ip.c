/*
 * $Id: nemesis-proto_ip.c,v 1.3 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
int
buildip(ETHERhdr *eth, IPhdr *ip, FileData *pd, FileData *ipod, char *device) 
{
	int n;
	...
	if (verbose == 2)
		nemesis_hexdump((char *)pkt, ip_packetlen, HEX_ASCII_DECODE);
	if (verbose >= 3)
		nemesis_hexdump((char *)pkt, ip_packetlen, HEX_RAW_DECODE);
	...
	if (n != ip_packetlen) {
		...
		if (verbose) {
			if (got_link)
				printf("Wrote %d byte IP packet through "
				    "linktype %s.\n", n, 
