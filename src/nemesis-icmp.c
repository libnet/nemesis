/*
 * $Id: nemesis-icmp.c,v 1.2 2005/09/27 19:46:19 jnathan Exp $
 *
 * THE NEMESIS PROJECT
...
void
nemesis_icmp(int argc, char **argv)
{
	...
	if (buildicmp(&etherhdr, &iphdr, &icmphdr, &ipunreach, &pd, &ipod, 
	    &origod, device) < 0) {
...
icmp_initdata(void)
{
	...
	ipunreach.ip_src.s_addr = 0;	/* ICMP unreach IP src addr. */
	ipunreach.ip_dst.s_addr = 0;	/* ICMP unreach IP dst addr. */
	ipunreach.ip_tos = 0;		/* ICMP unreach IP TOS */
	ipunreach.ip_id = (uint16_t)libnet_get_prand(PRu16);
					/* ICMP unreach IP ID */
	...
	icmphdr.hun.gateway = (uint32_t)libnet_get_prand(PRu32);
...
icmp_validatedata(void)
{
	...
	if (ipunreach.ip_src.s_addr == 0)
		ipunreach.ip_src.s_addr = (uint32_t)libnet_get_prand(PRu32);
	if (ipunreach.ip_dst.s_addr == 0)
		ipunreach.ip_dst.s_addr = (uint32_t)libnet_get_prand(PRu32);
	...
	if (got_mode > 1) {
		fprintf(stderr, "ERROR: ICMP injection mode multiply "
		    "specified - select only one.\n");
		icmp_exit(1);
	}
	...
	if (pd.file_s == 0 && 
		...
		udphdr.uh_dport = (uint16_t)libnet_get_prand(PRu16);
		udphdr.uh_ulen = htons(20);
		udphdr.uh_sum = 0;

		if ((pd.file_mem = (uint8_t *)calloc(8, 
		    sizeof(char))) == NULL) {
			perror("ERROR: Unable to allocate ICMP original "
			    "datagram payload memory");
			icmp_exit(1);
		}
		pd.file_s = 8;
	...
	/* 
	 * Attempt to send valid packets if the user hasn't decided to craft 
	 * an anomolous packet.
	 */
	switch (mode) {
...
static void
icmp_cmdline(int argc, char **argv)
{
	int opt, i;
	uint32_t addr_tmp[6];
	char *icmp_options;
	...
	while ((opt = getopt(argc, argv, icmp_options)) != -1) {
		...
		case 'a':	/* ICMP timestamp reply transmit time (epoch) */
			if (getint32(optarg, &icmphdr.dun.ts.its_ttime) < 0)
				icmp_usage(argv[0]);
			break;
		case 'A':	/* ICMP unreach original IP ID */
			if (getint16(optarg, &ipunreach.ip_id) < 0)
				icmp_usage(argv[0]);
			break;
		case 'b':	/* ICMP unreach original dst IP address */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&ipunreach.ip_dst.s_addr)) < 0) {
				fprintf(stderr, "ERROR: Invalid destination IP "
				    "address: \"%s\".\n", optarg);
				icmp_usage(argv[0]);
			}
			break;
		case 'B':	/* ICMP unreach original src IP address */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&ipunreach.ip_src.s_addr)) < 0) {
			...
			break;
		case 'c':	/* ICMP code */
			if (getint8(optarg, &icmphdr.icmp_code) < 0)
				icmp_usage(argv[0]);
			got_code = 1;
			break;
		...
		case 'D':	/* destination IP address */
			...
			break;
		case 'e':	/* ICMP ID */
			if (getint16(optarg, &icmphdr.hun.echo.id) < 0)
				icmp_usage(argv[0]);
			break;
		case 'f':	/* ICMP original datagram IP frag. offset */
			if (parsefragoptions(optarg, &ipunreach) < 0)
				icmp_usage(argv[0]);
			break;
		...
		case 'G':	/* ICMP redirect preferred gateway IP address */
			if ((nemesis_name_resolve(optarg, 
			    (uint32_t *)&icmphdr.hun.gateway)) < 0) {
				fprintf(stderr, "ERROR: Invalid preferred "
		...
		case 'H':	/* Ethernet source address */
			...
			break;
		case 'i':	/* ICMP type */
			if (getint8(optarg, &icmphdr.icmp_type) < 0)
				icmp_usage(argv[0]);
			got_type = 1;
			break;
		case 'I':	/* IP ID */
		...
		case 'j':	/* ICMP original datagram IP type of service */
			if (getint8(optarg, &ipunreach.ip_tos) < 0)
				icmp_usage(argv[0]);
			break;
		case 'J':	/* ICMP original datagram IP time to live */
			if (getint8(optarg, &ipunreach.ip_ttl) < 0)
				icmp_usage(argv[0]);
			break;
		case 'l':   /* ICMP unrechable original IP options file */
			if (strlen(optarg) < 256) {
				...
				fprintf(stderr, "ERROR: ICMP unreach original "
				    "IP options file %s > 256 characters.\n", 
				    optarg);
				icmp_usage(argv[0]);
			}
			break;
		case 'm':   /* mask for IP address mask messages */
			if (getint16(optarg, &icmphdr.dun.mask) < 0)
				icmp_usage(argv[0]);
			break;
		...
		case 'o':	/* ICMP timestamp originate time (epoch) */
			if (getint32(optarg, &icmphdr.dun.ts.its_otime) < 0)
				icmp_usage(argv[0]);
			break;
		...
		case 'p':	/* original IP protocol */
			if (getint8(optarg, &ipunreach.ip_p) < 0)
				icmp_usage(argv[0]);
			break;
		case 'P':	/* payload file */
			if (strlen(optarg) < 256) {
		...
		case 'r':	/* ICMP timestamp receive time (epoch) */
			if (getint32(optarg, &icmphdr.dun.ts.its_rtime) < 0)
				icmp_usage(argv[0]);
			break;
		case 's':	/* ICMP sequence number */
			if (getint16(optarg, &icmphdr.hun.echo.seq) < 0)
				icmp_usage(argv[0]);
			break;
